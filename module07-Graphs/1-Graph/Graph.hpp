...
/**
 * @brief CMP 246 Module 7 non-templated class representing a weighted graph 
 * G=(V, E).
 * 
 * This class uses an adjacency-list representation for the graph. The code is
...
private:
    /**
     * @brief (vertex, weight) pair stored in adjacency lists.
     * 
     * This class represents the (vertex, weight) pairs stored in the adjacency
     * list representation of the graph. Stream insertion is overloaded for
     * printing, and equality operation allows us to search a adjacency list
     * by vertex number.  
     */
    class AdjEntry {
    public:
        /**
         * @brief Construct a new Adj Entry object
         * 
         * @param vertex Vertex v in a (u, v) edge
         * @param weight Weight of the edge u, v
         */
        AdjEntry(size_t vertex, double weight) : v(vertex), wUV(weight) { }

        /**
         * @brief Vertex v in a (u, v) edge
         */
        size_t v;

        /**
         * @brief Weight of the edge (u, v)
         */
        double wUV;
         ...
         * 
         * @param vertex vertex number to compare to
         * @return true if this vertex number is equal to the parameter
         */
         ...
         * @param a AdjacencyEntry object to output
...
public:
    ...
    friend std::ostream& operator<<(std::ostream &out, const Graph &g) {
        for(size_t i = 1; i <= g.numV; i++) {
            out << i << ": [";
            auto itr = g.pAdjacents[i]->begin();
            while(itr != g.pAdjacents[i]->end()) {
                out << *itr << ", ";
                itr++;
            }
            out << "]" << std::endl;
...
private:
    ...
    /**
     * @brief Pointer to an array of pointers to STL lists, representing the 
     * adjacency lists for this graph. 
     * 
     */
    std::list<AdjEntry> **pAdjacents;
};
...
TEST_CASE("testing operator<<") {
    ...
    oss << g;
    std::string expected("1: [(2, 1), (5, 1), ]\n");
    expected += "2: [(1, 1), (5, 1), (4, 1), (3, 1), ]\n";
    expected += "3: [(2, 1), (4, 1), ]\n"; 
    expected += "4: [(2, 1), (3, 1), (5, 1), ]\n";
    expected += "5: [(1, 1), (2, 1), (4, 1), ]\n";
