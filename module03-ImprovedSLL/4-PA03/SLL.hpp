...
public:
     ...
     * 
     * Override of the compount addition / assignment operator to work with SLL
     * objects. Adds the type T value on the right side of the operator to the 
     * front of this list.
     * 
     * @param value Type T value to add to the front of the list.
     * 
     * @return Reference to this object.
     */
    SLL<T> &operator+=(const T& value);

    // TODO: write Doxygen comment header and function declaration for the 
    // -= operator override

    // TODO: write Doxygen comment header for [] operator override. Do not 
    // forget the @throws tag!
...
TEST_CASE("testing SLL<T> assignment") {
    ...
    // right size?
    CHECK(list1.size() == list2.size());
    CHECK(list2.size() == list3.size());

    // same contents?
    for(unsigned i = 0; i < list1.size(); i++) {
        CHECK(list1.get(i) == list2.get(i));
        CHECK(list1.get(i) == list3.get(i));
    }
...
// TODO: write the implementation of the += override


// doctest unit test for the += operator
TEST_CASE("testing SLL<T> += operator") {
    SLL<int> list;
...
// doctest unit test for the -= override
TEST_CASE("testing SLL<T> -= operator") {
    SLL<int> list;
    for(int i = 9; i >= 0; i--) {
        list.add(i);
...
// doctest unit tests for [] operator
TEST_CASE("testing SLL<T> [] operator") {
    SLL<char> list;
    ...
    // [] should allow changes to list elements
    list[13] = 'm';
    CHECK(list[13] == 'm');
    ...
    bool flag = true;
    try {
        list[26] = 'z'; // list element 26 does not exist
        flag = false; // this line should not be reached, due to an exception
    } catch(std::out_of_range oor) {
        // verify flag wasn't modified
        CHECK(flag);
