...
public:
     ...
     */
    SimpleSLL() : pHead(0), n(0), pPrng(new std::mt19937_64(time(0))) { }
    ...
    // TODO: Complete Doxygen comments for getRandom(). 
    // Use the other Doxygen comments in this class for examples.
    T getRandom() const;
...
private:
    ...
    /**
     * Pseudo-random number generator to use in getRandom().
     */
    std::mt19937_64 *pPrng;
};
...
// TODO: Complete the code for getRandom
/*
 * Get a random element from the list. 
 */
template <class T>
T SimpleSLL<T>::getRandom() const {
    // stub - replace with your own code
    return 0;
}

// doctest unit test for the getRandom method
TEST_CASE("testing SimpleSLL<T>::getRandom") {
    // populate a large list with random numbers
    std::mt19937_64 *pPrng = new std::mt19937_64(time(0));
    std::uniform_real_distribution<double> dist(0, 1);

    SimpleSLL<double> list;
    for(int i = 0; i < 1000000; i++) {
        list.add(dist(*pPrng));
    }

    // now draw 5 at random -- they should all be different
    std::set<double> checkSet;
    std::set<int> idxSet;
    for(int i = 0; i < 5; i++) {
        double v = list.getRandom();
        int idx = list.contains(v);
        checkSet.insert(v);
        idxSet.insert(idx);
    }

    // make sure 5 different elements were returned
    CHECK(checkSet.size() == 5);
    // make sure they were reasonably far apart
